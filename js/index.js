...
for (var i = 0; i<cityInfo.length; i++) {
        ...
        txt = document.createTextNode(cityInfo[i]);
    option.appendChild(txt);
    option.setAttribute("value",cityInfo[i]);
    citySel.insertBefore(option,citySel.lastChild);
    }

var majorSel = document.getElementById("majorSel");
//load cities 
for (var i = 0; i<majors.length; i++) {
        var option = document.createElement("OPTION"),
        txt = document.createTextNode(majors[i]);
    option.appendChild(txt);
    option.setAttribute("value",majors[i]);
    majorSel.insertBefore(option,majorSel.lastChild);
    }
